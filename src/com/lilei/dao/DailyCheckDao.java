...
public class DailyCheckDao {
	...
	public int getTotalTrainingDays(String userId) {
		if (getTotalTrainingTimes(userId) == 0) {
			TrainDao trainDao = new TrainDao();
			trainDao.addNewTrainRecord(userId, 0+"");
		}
	...
	public int getTotalTrainingTimes(String userId) {
		sql = "SELECT COUNT(*) FROM training WHERE userId = ?;";
		try {
			return queryRunner.query(sql, new ScalarHandler<Long>(), userId).intValue();
		} catch (SQLException e) {
			return 0;
